...
const Products = ({
  ...
  console.log(products.find(p => p.category))
  console.log(categories.find(c => c._id))

  const propertiesToFill = [];
  if (categories.length > 0 && category) {
    let selectCat = categories.find(({ _id }) => _id === category);
    propertiesToFill.push(...selectCat.properties);
  }
    return (
    <Layout>
      <Link
