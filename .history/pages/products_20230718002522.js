...
const Products = ({
  ...
  if (products.length > 0 && categories) {
    let selectCatInfo = products.find(({ name }) => name === categories);
    propertiesToFill.push(...selectCatInfo.name);
    while (selectCatInfo?._name) {
      const parentCat = products.find(
        ({ _id }) => _id === selectCatInfo.name
      );
      propertiesToFill.push(...parentCat.name);
