...
export const CardOrder = () => {
  ...
  for (const productId of cartProducts) {
    const price = matchingProducts.find((p) => p._id == productId)?.price || 0;
  ...
  function countQty(_id) {
    const qty = cartProducts?.filter((id) => id === _id);
  ...
  const calculateTotalPrice = () => {
    let subtotal = 0;
    for (const productId of cartProducts) {
      if (matchingProducts.find((p) => p._id == productId)?.discount) {
        const price = matchingProducts.find((p) => p._id == productId)?.price;
        const discount =
          price -
          (price * matchingProducts.find((p) => p._id == productId).discount) /
            100;
        subtotal += Number(discount.toFixed(2));
      } else {
        const price =
          matchingProducts.find((p) => p._id == productId)?.price || 0;
        subtotal += Number(price?.toFixed(2));
      }
    }
  ...
  function eachItemtotalPrice(_id, price) {
    const totalItems = cartProducts.filter((id) => id === _id);
    const total = totalItems.length * price;
    return total;
  }

  const discountTotal = (_id, price, discount) => {
    const formattedNum =
      eachItemtotalPrice(_id, price) -
      (eachItemtotalPrice(_id, price) * discount) / 100;
  ...
  const discount = () => {
    const dicount = subtotal - calculateTotalPrice();
    return Number(dicount.toFixed(2));
  };
  ...
  function increaseProduct(id) {
    ...
    setMatchingProducts((prevProducts) =>
      prevProducts.map((product) =>
        product._id === id ? { ...product, discount: {discount: 0} } : product
      )
    );
  ...
  console.log(matchingProducts)
   ...
   function decreaseProduct(id, discount) {
    removeProduct(id);
              ...
              <tr className="border-b-2" key={p._id}>
                ...
                <td className="text-center">
                  <p>
                    $
                    {p.discount
                      ? discountTotal(p._id, p.price, p.discount)
                ...
                <td className="p-4 w-[90px] text-center">
                  <input
                    ...
                    value={p.dicount}
                    onChange={(e) =>
                      setDiscount(p._id, parseInt(e.target.value))
                    }
                  />
                </td>
                <td className="flex items-center justify-between my-4 px-4">
                  ...
                  <button
                    ...
                    onClick={() => increaseProduct(p._id, p.discount)}
                  >
                    +
                  </button>
                </td>
