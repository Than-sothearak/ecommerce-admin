...
export const CardOrder = () => {
  ...
  useEffect(() => {
    ...
    const foundProducts = products.filter(product => cartProducts.includes(product._id));
    setMatchingProducts(foundProducts);
  }, [cartProducts]);
  
  function findProductsByIds(idArray, products) {
    return products.filter(product => idArray.includes(product._id));
  }
  ...
  const calculateTotalPrice = () => {
    ...
    for (const productId of cartProducts) {
      ...
      if (matchingProducts.find((p) => p._id == productId)?.discount) {
        ...
        const discount = price - price * matchingProducts.find(p => p._id == productId).discount / 100
