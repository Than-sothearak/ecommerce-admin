 ...
 const handleClear = () => {
  ...
  const calculateTotalPrice = () => {
    ...
    for (const productId of cartProducts) {
      ...
      const discountedPrice = products.discount ?  subtotal - (subtotal * products.discount) / 100 : subtotal;
      totalPrice += discountedPrice;
    }
    return Number(totalPrice.toFixed(2));
  };
  
  let num = subtotal - calculateTotalPrice()
  const discountTotalNum = Number(num.toFixed(2))
